...
export const POST=async({request})=>{try{
  const body=await request.json(); const target=String(body.target||'').trim().slice(0,500); const description=String(body.description||'').trim().slice(0,12000); const chainId=Number(body.chainId)||11155111;
