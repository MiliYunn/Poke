...
<main class="route-page plans-page">
  <section class="plans-hero"><span class="eyebrow">SIMPLE, EVIDENCE-FIRST PRICING</span><h1>Start free.<br/><em>Protect more with Premium.</em></h1><p>Portfolio and Explorer remain open to everyone. Plans include separate monthly allowances for AI-assisted scam scans and tamper-evident whistleblower reports.</p><div class="current-plan"><span>Current plan</span><strong>{plan==='premium'?'Premium':'Free'}</strong><small>{scamUsage} scans · {usage} reports used this month</small></div></section>
  {#if changed}<div class="plan-toast">Plan updated for this MVP device.</div>{/if}
  <section class="pricing-grid">
    <article class:current={plan==='free'}><div class="plan-top"><span class="eyebrow">FREE</span>{#if plan==='free'}<b>Current</b>{/if}</div><h2>For occasional verification</h2><div class="price"><strong>RM0</strong><span>/ month</span></div><p>Explore Web3, check suspicious targets, and preserve important reports.</p><ul><li><i>✓</i><span><strong>5 AI scam scans</strong> per month</span></li><li><i>✓</i><span><strong>5 whistleblower reports</strong> per month</span></li><li><i>✓</i><span><strong>5 evidence files</strong> per report</span></li><li><i>✓</i><span>Cross-chain portfolio access</span></li><li><i>✓</i><span>Multi-chain block explorer</span></li></ul><button onclick={()=>choose('free')} disabled={plan==='free'}>{plan==='free'?'Current plan':'Switch to Free'}</button></article>
    <article class="premium" class:current={plan==='premium'}><div class="recommended">RECOMMENDED</div><div class="plan-top"><span class="eyebrow">PREMIUM</span>{#if plan==='premium'}<b>Current</b>{/if}</div><h2>For active investigators</h2><div class="price"><strong>RM29</strong><span>/ month</span></div><p>Run more analyses and preserve more evidence with expanded AI context.</p><ul><li><i>✓</i><span><strong>100 AI scam scans</strong> per month</span></li><li><i>✓</i><span><strong>100 whistleblower reports</strong> per month</span></li><li><i>✓</i><span><strong>20 evidence files</strong> per report</span></li><li><i>✓</i><span>Expanded Gonka chatbot usage</span></li><li><i>✓</i><span>Exportable verification records</span></li></ul><button onclick={()=>choose('premium')} disabled={plan==='premium'}>{plan==='premium'?'Current plan':'Try Premium in MVP'}</button></article>
  </section>
