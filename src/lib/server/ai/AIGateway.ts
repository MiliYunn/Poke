...
export class AIGateway {
  ...
  async chat(messages: Array<{role: string; content: string}>, stream = false, model = this.primaryModel) {
    ...
    try { return await fetch(`${this.baseUrl}/chat/completions`, { method: 'POST', headers: { 'content-type':'application/json', authorization:`Bearer ${env.GONKA_API_KEY}` }, body: JSON.stringify({ model, messages, stream, temperature:0.1, max_tokens:1800 }), signal:controller.signal }); }
